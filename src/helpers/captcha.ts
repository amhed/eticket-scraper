...
export async function solveCaptcha(page: Puppeteer.Page) {
  try {
    await page.waitForFunction(() => {
      const iframe = document.querySelector('iframe[src*="api2/anchor"]') as any
    ...
    const recaptchaFrame = frames.find(frame => frame.url().includes('api2/anchor')) as any

    const checkbox = await recaptchaFrame.$('#recaptcha-anchor')
    await checkbox.click({ delay: rdn(30, 150) })

    await page.waitForFunction(() => {
      const iframe = document.querySelector('iframe[src*="api2/bframe"]') as any
      if (!iframe) return false
    ...
    const imageFrame = frames.find(frame => frame.url().includes('api2/bframe')) as any
    ...
    while (true) {
      ...
      try {
        ...
        return page.evaluate(() => (document.getElementById('g-recaptcha-response')as any).value)
      } catch (e) {
        console.error(e)
        continue
      }
